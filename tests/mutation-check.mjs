...
const mutations = [
  ...
  },
  {
    ...
    from: "  const cap = Number.isFinite(wanted) && wanted > 0 ? wanted : 0;",
    to: "  const cap = Number.isFinite(wanted) && wanted > 0 ? wanted : 96;",
  },
  {
  ...
  },
  {
    ...
    from: "  return rest.slice(1).every((t) => !t.text.startsWith(\"-\") || PROBE_FLAG_RE.test(t.text));",
    to: "  return false;",
  },
  {
  ...
  },
  {
    ...
    suite: "tests/t-static.mjs",
    expect: "readOnlyDirs: a delete inside a read-only dir is outside the project",
    from: '  if (inReadOnlyScope(lower, scope)) return { kind: "outside", path: raw };',
    to: '  if (false) return { kind: "outside", path: raw };',
  },
  {
    name: "a block reason loses its near-miss alternative",
    suite: "tests/t-static.mjs",
    expect: "near miss: an outside delete names the scope that would have allowed it",
    from: "  const near = opts.nearMiss ? ` ${opts.nearMiss}` : \"\";",
    to: '  const near = "";',
  },
  {
    name: "a half-written audit tail is chained onto anyway",
    suite: "tests/t-coverage.mjs",
    expect: "audit: a half-written tail is quarantined instead of extended",
    from: "  if (state.healthy) return state.chain;",
    to: "  if (true) return state.chain;",
  },
  {
    name: "a blocked call that ran is no longer linked",
    suite: "tests/t-coverage.mjs",
    expect: "outcome: a blocked call that comes back through a tool result is recorded",
    from: "  const link = blockedCallLinks.get(key);\n  if (!link) return false;",
    to: "  const link = blockedCallLinks.get(key);\n  if (true) return false;",
  },
  {
    name: "the degraded list stops recording what could not be enforced",
    suite: "tests/t-coverage.mjs",
    expect: "degraded: the hub payload appears in the live list",
    from: 'function degrade(code, detail = "") {\n  const key = String(code ?? "").trim();',
    to: 'function degrade(code, detail = "") {\n  if (code) return;\n  const key = String(code ?? "").trim();',
  },
  {
    name: "deny-and-abort stops holding the hard preset",
    suite: "tests/t-ui.mjs",
    expect: "deny & abort: the hard preset is the floor until /dc is opened",
    from: "  if (lockdownActive()) {\n    const hard = MODE_PRESETS.hard[rule] ?? \"block\";",
    to: "  if (false) {\n    const hard = MODE_PRESETS.hard[rule] ?? \"block\";",
  },
  {
    name: "a command that does not fit the prompt budget is judged anyway",
    suite: "tests/t-llm.mjs",
    expect: "prompt budget: a command that does not fit is refused, not trimmed",
    from: "  return prompt.includes(actionLine);",
    to: "  return true;",
  },
  {
    name: "the session context block is never sent",
    suite: "tests/t-llm.mjs",
    expect: "context: the session block is sent inside an untrusted wrapper",
    from: '  if (!CFG.checker.includeContext) return "";',
    to: '  if (true) return "";',
  },
  {
    name: "the CLI checker stops reusing its child",
    suite: "tests/t-llm.mjs",
    expect: "cli child: a second check reuses the same child",
    from: "  if (checkerChild && checkerChild.key === key && !checkerChild.dead) return checkerChild;",
    to: "  if (false) return checkerChild;",
  },
  {
    name: "the config is never re-read after the first load",
    suite: "tests/t-coverage.mjs",
    expect: "freshness: a hand-edited policy takes effect without a restart",
    from: "  if (!force && now - configCheckedAt < FRESHNESS_TTL_MS) return false;",
  ...
  },
  {
    ...
    suite: "tests/t-static.mjs",
    expect: "secret write blocked: .env in the project",
    from: '      for (const target of fileToolTargets(input, text)) violations.push(...writeTargetViolations(target, scope, "", { secrets, text }));',
    to: '      for (const target of fileToolTargets(input, text)) violations.push(...writeTargetViolations(target, scope, "", { secrets: false, text }));',
  },
  {
    name: "redirect destinations are not classified again",
    suite: "tests/t-static.mjs",
    expect: "write blocked: a redirect leaving the project",
    from: '    for (const dest of redirectTargets(part)) out.push(...writeTargetViolations(dest, current, ">", layers));',
    to: '    for (const dest of []) out.push(...writeTargetViolations(dest, current, ">", layers));',
  },
  {
    name: "allowDirs entries are trusted as written again",
    suite: "tests/t-static.mjs",
    expect: "allowDirs: /dc status reports the rejected entry and the reason",
    from: "    const reason = allowDirReject(text);",
    to: '    const reason = "";',
  },
  {
  ...
  },
  {
    ...
    from: "  for (const body of bodies) if (body.code) out.push(...writeViolations(body.text, scope, depth + 1, layers));",
    to: "  for (const body of bodies) if (false) out.push(...writeViolations(body.text, scope, depth + 1, layers));",
  },
  {
  ...
  },
  {
    ...
    from: "  const head = policyBlock(Math.floor(CFG.maxPromptChars * 0.6));",
    to: '  const head = "";',
  },
  {
  ...
  {
    name: "the guard's own control files are editable again",
    suite: "tests/t-static.mjs",
    expect: "guardSelf: writing the guard's config is blocked",
    from: "    if (candidate === lower || underDir(candidate, lower)) return path;",
    to: "    if (false) return path;",
  },
  {
    name: "a floored rule can be set to allow",
    suite: "tests/t-static.mjs",
    expect: "guardSelf: `allow` in the file does not open the rule (RULE_FLOORS)",
    from: "    if ((ACTION_RANK[rules[rule]] ?? 0) > (ACTION_RANK[floor] ?? 0)) {",
    to: "    if (false) {",
  },
  {
    name: "the read-only class no longer short-circuits the scanners",
    suite: "tests/t-static.mjs",
    expect: "read-only class: `git clean -n` only prints and passes without a model",
    from: "  if (readOnlyCommand(command)) return out;",
    to: "  if (false) return out;",
  },
  {
    name: "the readonly gate stops gating",
    suite: "tests/t-static.mjs",
    expect: "readonly: a directory create blocks without a model call",
    from: "  if (CFG.mode === \"readonly\" && !adapter.readOnly(ex, scope, input)) {",
    to: "  if (false) {",
  },
  {
    name: "the hub denylist stops denying",
    suite: "tests/t-coverage.mjs",
    expect: "hub: curl is refused before anything runs",
    from: "  if (HUB_DENIED_APPLICATIONS.test(name)) {",
    to: "  if (false) {",
  },
  {
    name: "a loosening project entry is accepted instead of refused",
    suite: "tests/t-coverage.mjs",
    expect: "project policy: every refused key is reported",
    from: "    if ((ACTION_RANK[wanted] ?? 0) >= (ACTION_RANK[CFG.rules[rule]] ?? 0)) {",
    to: "    if (false) {",
  },
  {
    name: "the project policy stops tightening the effective action",
    suite: "tests/t-coverage.mjs",
    expect: "project policy: a project can tighten a rule",
    from: "  if (scoped && (ACTION_RANK[scoped] ?? 0) < (ACTION_RANK[action] ?? 0)) action = scoped;",
    to: "  if (false) action = scoped;",
  },
  {
    name: "a read mark survives the file changing",
    suite: "tests/t-coverage.mjs",
    expect: "unreadTarget: a file that changed since the read is unread again",
    from: "  if (mark && mark.size === stat.size && mark.mtimeMs === stat.mtimeMs) return [];",
    to: "  if (mark) return [];",
  },
  {
    name: "the fast stage accepts any answer",
    suite: "tests/t-llm.mjs",
    expect: "two-stage: an answer that is not 0 or 1 is a checker failure, not an allow",
    from: "  if (digit !== \"1\") {",
    to: "  if (false) {",
  },
];
